"""
Classify dentin test images and write predictions CSV.

Output format mirrors external_test_predictions.csv:
  file,class_name,target,target_label,pred_idx,pred_label,prob_positive

For this 3-class model (dentin/enamel/pulp), prob_positive is P(dentin).
...
INPUT_DIR = "image-testing/dentin_test"
OUTPUT_DIR = "test_predictions"
OUTPUT_CSV = os.path.join(OUTPUT_DIR, "dentin_test_predictions.csv")
...
def main() -> None:
    ...
    for name in image_names:
        ...
        print(
            ...
            f"P(dentin)={probs[TARGET_IDX]:.4f}"
        )
    ...
    print(f"Accuracy on dentin_test: {correct}/{len(rows)} ({accuracy:.1f}%)")
    print(
        "Note: prob_positive = P(dentin) for this 3-class model "
        "(dentin / enamel / pulp)."
    )
